...
dataset_path = './/combined_prepared_dataset//'
# print('Creating Directory: ' + dataset_path)
# os.makedirs(dataset_path, exist_ok=True)

# tmp_fake_path = './/data01_tmp_fake_faces'
...
all_fake_faces = [f for f in sorted(os.listdir(fake_path)) if os.path.isfile(os.path.join(fake_path, f))]
...
# all_fake_faces = too_many_fake_faces[0:len(all_real_faces)]
# for fname in random_faces:
#     src = os.path.join(tmp_fake_path, fname)
#     dst = os.path.join(fake_path, fname)
#     shutil.copyfile(src, dst)
...
split_path = './/combined_split_dataset//'
...
for i in range(len(all_real_faces)):
    ...
    last_real = real_file_name
    last_fake = fake_file_name
    if i > test_val_tgth:
        # then it's training
